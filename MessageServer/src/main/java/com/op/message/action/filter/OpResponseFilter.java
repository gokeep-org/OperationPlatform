...
public class OpResponseFilter implements ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext) throws IOException {
        if (responseContext.getStatus() == 200)
            LOGGER.info("message server request status code is :" + responseContext.getStatus());
        else
            LOGGER.error("message server request error code is :" + responseContext.getStatus());
    }
}