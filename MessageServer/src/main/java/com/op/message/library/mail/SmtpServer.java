...
public class SmtpServer {
    ...
    public SmtpServer setAttachment() throws MessagingException {
        ...
        if (!OpUtils.checkObjectIsNull(email.getAttachmentPath())) {
            email.getAttachmentPath().forEach(e -> {
                File file = new File(e);
                sources.add(new FileDataSource(file));
            });
        }
        if (!OpUtils.checkObjectIsNull(email.getAttachmentFile())) {
            email.getAttachmentFile().forEach(e -> {
